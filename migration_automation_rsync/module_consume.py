...
from migration_automation_rsync.module_rsync import ModuleRsync
...
class ModuleConsume:
    ...
    def __init__(self, queue, development=None):
        ...
        self.module_rsync = ModuleRsync(self.module_log.log, self.module_configuration)
    ...
    def config_init(self):
        ...
        try:
            ...
            if not self.module_configuration.initialize():
                ...
                # instance the ModuleLog class to Rsync path log (3)
        ...
        **values** - dict with message value
        ...
        try:
            ...
            if properties.headers.get("x-key") is not None and \
                ...
                if self.module_rsync.initialize(self._values):
                    # run process in rsync
                    self.module_rsync.process()
                    # info log completed process
                    self._log.info("Rsync process completed - {}".format(self._values))
            else:
                # error log with invalid key
                self._log.error("Invalid key rsync - {}".format(self._values))

        except Exception as er:
            # generate a error log
            self._log.error("Rsync - {} - {}".format(self.__class__.__name__, er))
            return False
