...
from migration_automation_check_end.module_check_end import ModuleCheckEnd
...
class ModuleConsume:
    ...
    def __init__(self, queue, development=None):
        ...
        self.module_check_end = ModuleCheckEnd(self.module_log.log, self.module_configuration)
    ...
    def config_init(self):
        ...
        try:
            ...
            if not self.module_configuration.initialize():
                ...
                # instance the ModuleLog class to Check End path log (7)
                self.module_log = ModuleLog(self.module_configuration, 7)
        ...
        **values** - dict with message value
        ...
        try:
            ...
            if properties.headers.get("x-key") is not None and \
                ...
                if self.module_check_end.initialize(self._values):
                    # run process in check
                    self.module_check_end.process()
                    # info log completed process
                    self._log.info("Check End process completed - {}".format(self._values))
            else:
                # error log with invalid key
                self._log.error("Invalid key check end - {}".format(self._values))

        except Exception as er:
            # generate a error log
            self._log.error("Check End - {} - {}".format(self.__class__.__name__, er))
            return False
