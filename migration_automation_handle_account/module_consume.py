...
from migration_automation_handle_account.module_handle_account import ModuleHandleAccount
...
class ModuleConsume:
    ...
    def __init__(self, queue, development=None):
        ...
        self.module_handle_account = ModuleHandleAccount(self.module_log.log, self.module_configuration)
    ...
    def config_init(self):
        ...
        try:
            ...
            if not self.module_configuration.initialize():
                ...
                # instance the ModuleLog class to Handle Account path log (11)
                self.module_log = ModuleLog(self.module_configuration, 11)
        ...
        **values** - dict with message value
        ...
        try:
            ...
            if properties.headers.get("x-key") is not None and \
                ...
                if self.module_handle_account.initialize(self._values):
                    # run process in Handle Account
                    self.module_handle_account.process()
                    # info log completed process
                    self._log.info("Handle Account process completed - {}".format(self._values))
            else:
                # error log with invalid key
                self._log.error("Invalid key Handle Account - {}".format(self._values))

        except Exception as er:
            # generate a error log
            self._log.error("Handle Account - {} - {}".format(self.__class__.__name__, er))
            return False
