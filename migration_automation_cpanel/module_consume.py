...
from migration_automation_cpanel.module_cpanel import ModuleCPanel
...
class ModuleConsume:
    ...
    def __init__(self, queue, development=None):
        ...
        self.module_cpanel = ModuleCPanel(self.module_log.log, self.module_configuration)
    ...
    def config_init(self):
        ...
        try:
            ...
            if not self.module_configuration.initialize():
                ...
                # instance the ModuleLog class to CPanel path log (2)
        ...
        **values** - dict with message value
        ...
        try:
            ...
            if properties.headers.get("x-key") is not None and \
                ...
                if self.module_cpanel.initialize(self._values):
                    # run process in cpanel
                    self.module_cpanel.process()
                    # info log completed process
                    self._log.info("CPanel process completed - {}".format(self._values))
            else:
                # error log with invalid key
                self._log.error("Invalid key cpanel - {}".format(self._values))

        except Exception as er:
            # generate a error log
            self._log.error("CPanel - {} - {}".format(self.__class__.__name__, er))
            return False
