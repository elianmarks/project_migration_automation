...
from migration_automation_compare.module_compare import ModuleCompare
...
class ModuleConsume:
    ...
    def __init__(self, queue, development=None):
        ...
        self.module_compare = ModuleCompare(self.module_log.log, self.module_configuration)
    ...
    def config_init(self):
        ...
        try:
            ...
            if not self.module_configuration.initialize():
                ...
                # instance the ModuleLog class to Compare path log (8)
                self.module_log = ModuleLog(self.module_configuration, 8)
        ...
        **values** - dict with message value
        ...
        try:
            ...
            if properties.headers.get("x-key") is not None and \
                ...
                if self.module_compare.initialize(self._values):
                    # run process in check
                    self.module_compare.process()
                    # info log completed process
                    self._log.info("Compare process completed - {}".format(self._values))
            else:
                # error log with invalid key
                self._log.error("Invalid key compare - {}".format(self._values))

        except Exception as er:
            # generate a error log
            self._log.error("Compare - {} - {}".format(self.__class__.__name__, er))
            return False
