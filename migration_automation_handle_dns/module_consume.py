...
from migration_automation_handle_dns.module_handle_dns import ModuleHandleDNS
...
class ModuleConsume:
    ...
    def __init__(self, queue, development=None):
        ...
        self.module_handle_dns = ModuleHandleDNS(self.module_log.log, self.module_configuration)
    ...
    def config_init(self):
        ...
        try:
            ...
            if not self.module_configuration.initialize():
                ...
                # instance the ModuleLog class to HandleDNS path log (10)
                self.module_log = ModuleLog(self.module_configuration, 10)
    ...
    def connection(self):
        ...
        **channel** - channel of the connection
        ...
        try:
            ...
            self._log.error("Handle DNS - {} - {}".format(self.__class__.__name__, er))
            return False
        ...
        **values** - dict with message value
        ...
        try:
            ...
            if properties.headers.get("x-key") is not None and \
                ...
                if self.module_handle_dns.initialize(self._values):
                    # run process in Handle DNS
                    self.module_handle_dns.process()
                    # info log completed process
                    self._log.info("Handle DNS process completed - {}".format(self._values))
            else:
                # error log with invalid key
                self._log.error("Invalid key Handle DNS - {}".format(self._values))
