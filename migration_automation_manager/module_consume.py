...
from migration_automation_manager.module_manager import ModuleManager
...
class ModuleConsume:
    ...
    def __init__(self, queue, development=None):
        ...
        self.module_manager = ModuleManager(self.module_log.log, self.module_configuration)
    ...
    def config_init(self):
        ...
        try:
            ...
            if not self.module_configuration.initialize():
                ...
                # instance the ModuleLog class to Manager path log (6)
                self.module_log = ModuleLog(self.module_configuration, 6)
        ...
        **values** - dict with message value
        ...
        try:
            ...
            if properties.headers.get("x-key") is not None and \
                ...
                if self.module_manager.initialize(self._values):
                    # run process in check
                    self.module_manager.process()
                    # info log completed process
                    self._log.info("Manager process completed - {}".format(self._values))
            else:
                # error log with invalid key
                self._log.error("Invalid key manager - {}".format(self._values))

        except Exception as er:
            # generate a error log
            self._log.error("Manager - {} - {}".format(self.__class__.__name__, er))
            return False
